...
class FD_eulerimp:
	...
	def pricer_call(self):
		...
		for n in range(0,self.nt):
			vec=np.zeros(self.nx-2)
			vec[-1]=-c*(np.exp(0.5*(k+1)*x[-1]+0.25*(k+1)**2*(n*dt))-np.exp(0.5*(k-1)*x[-1 ]+0.25*(k-1)**2*(n*dt)))
			T=np.dot(np.linalg.inv(mat_imp),np.transpose(T-vec))
	...
	def pricer_put(self):
		...
		mat_imp=np.zeros((self.nx,self.nx))
		...
		mat_imp[0,1]=m3[1]
		mat_imp[1,2]=m3[1]
		mat_imp[self.nx-2,self.nx-3]=m2[self.nx-1]
		mat_imp[self.nx-2,self.nx-2]=m1[self.nx-1]
		mat_imp[self.nx-1,self.nx-1]=1
		for k in range(1,self.nx-2):
			mat_imp[k,k-1]=m2[k+1]
			mat_imp[k,k]=m1[k+1]
			mat_imp[k,k+1]=m3[k+1]

		sol_bs=[np.maximum(1-vec_x[j],0) for j in range(0,self.nx+1)]
		val=sol_bs[1:self.nx+1]
		inv_mat_imp=np.linalg.inv(mat_imp)
		for n in range(1,self.nt):
			c=np.zeros(self.nx)
			c[0]=(-m3[self.nx-1])*np.exp(-self.mu*(self.maturity-(n+1)*pas_t))
			val=np.dot(inv_mat_imp,np.transpose(val+c))
#		print(val)
